...
class Database:
    ...
    def insert_data_pi(self, data_dict: pd.DataFrame) -> None:
        self.log.info('INSERT INTO %s', self.conf.table_pi)
        page_size: int = 10000
        with self.connection.cursor() as cursor:
            psycopg2.extras \
                .execute_values(cursor,
                                """ INSERT INTO """ + self.conf.table_pi +
                                """ ( fecha,
                                      ads,
                                      new,
                                      marca
                                ...
                                page_size=page_size)
            self.log.info('INSERT %s COMMIT.', self.conf.table_pi)
            self.connection.commit()
            self.log.info('CLOSE CURSOR %s', self.conf.table_pi)
            cursor.close()

    def insert_data_cl_autos(self, data_dict: pd.DataFrame) -> None:
        self.log.info('INSERT INTO %s', self.conf.table_cl_autos)
        page_size: int = 10000
        with self.connection.cursor() as cursor:
            psycopg2.extras \
                .execute_values(cursor,
                                """ INSERT INTO """ + self.conf.table_cl_autos +
                                """ ( fecha,
                                      ads,
                                      new,
                                      automotora,
                                ...
                                page_size=page_size)
            self.log.info('INSERT %s COMMIT.', self.conf.table_cl_autos)
            self.connection.commit()
            self.log.info('CLOSE CURSOR %s', self.conf.table_cl_autos)
            cursor.close()
